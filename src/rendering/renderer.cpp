...
Renderer::~Renderer()
{
    ...
    if (m_vboSdf)
        glDeleteBuffers(1, &m_vboSdf);
    if (m_vaoSdf)
        glDeleteVertexArrays(1, &m_vaoSdf);
    
    if (m_vboRenderSdf)
        glDeleteBuffers(1, &m_vboRenderSdf);
    if (m_vaoRenderSdf)
        glDeleteVertexArrays(1, &m_vaoRenderSdf);
...
void Renderer::createSdfVAO(GLuint vbo, int numParticles)
{
    if (m_vboSdf != vbo)
        glDeleteBuffers(1, &m_vboSdf);
    if (m_vaoSdf)
    ...
    m_vboSdf = vbo;
    m_numParticlesSdf = numParticles;
    
    // Initialize the vertex array object.
    glGenVertexArrays(1, &m_vaoSdf);
    glBindVertexArray(m_vaoSdf);
    
    // bind vertex buffer object.
    glBindBuffer(GL_ARRAY_BUFFER, m_vboSdf);
...
void Renderer::createRenderSdfVAO()
{
    GLfloat sdfRenderVertices[18]
            {
                    -1.f, 1.f, -1.f,
                    -1.f, -1.f, -1.f,
                    1.f, 1.f, -1.f,
    ...
    glGenVertexArrays(1, &m_vaoRenderSdf);
    glBindVertexArray(m_vaoRenderSdf);

    glGenBuffers(1, &m_vboRenderSdf);

    glBindBuffer(GL_ARRAY_BUFFER, m_vboRenderSdf);
    
    glBufferData(GL_ARRAY_BUFFER, 18 * sizeof(GLfloat), sdfRenderVertices, GL_STATIC_DRAW);
    
    GLuint position = glGetAttribLocation(m_programSdf, "position");
...
void Renderer::render(std::vector<int2> colorIndices, std::vector<float4> colors)
{
    ...
    if (m_toggleRenderSdf)
    {
        ...
        if (m_sdfSceneID != 0)
        {
            glUseProgram(m_programSdf);
            glBindVertexArray(m_vaoRenderSdf);
        
            glm::vec3 eye = glm::vec3(m_camera->getEye());
            glUniform3f(glGetUniformLocation(m_programSdf, "eye"), eye.x, eye.y, eye.z);
            glUniform4f(glGetUniformLocation(m_programSdf, "color"), 1.f, 1.f, 1.f, 1.f);
            glUniformMatrix4fv(glGetUniformLocation(m_programSdf, "VPI"), 1, GL_FALSE, glm::value_ptr(glm::inverse(projView)));
            glUniformMatrix4fv(glGetUniformLocation(m_programSdf, "VP"), 1, GL_FALSE, glm::value_ptr(projView));
    ...
    else
    {
        ...
        glDrawArrays(GL_POINTS, 0, m_numParticlesSdf);
...
{
    m_program = _compileProgram("../res/shaders/shader.vert", "../res/shaders/shader.frag");
}
...
void Renderer::keyPressed(QKeyEvent *e)
{
    ...
    case Qt::Key_Q:
        ...
        break;
    case Qt::Key_T:
        m_toggleRenderSdf = !m_toggleRenderSdf;
        break;
    default:
        break;
    }
...
void Renderer::setNumParticlesSdf(int num)
{
    this->m_numParticlesSdf = num;
}

void Renderer::setSdfSceneID(int id)
{
    this->m_sdfSceneID = id;
    const char* vertex = "../res/shaders/sdf.vert";
    switch (m_sdfSceneID)
    {
        case 1:
            // test scene: precomputed sphere
            m_programSdf = _compileProgram(vertex, "../res/shaders/sdf1.frag");
            break;
        
        case 2:
            // test scene: dynamic box
            m_programSdf = _compileProgram(vertex, "../res/shaders/sdf2.frag");
            break;

        case 3:
            // cubes and spheres scene
            m_programSdf = _compileProgram(vertex, "../res/shaders/sdf3.frag");
            break;
